...
const PhdProffesors = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(`${URL}/phdProffesors/fetch`);
  ...
  const deleteCustomer = async (id) => {
    try {
      const response = await axios.delete(`${URL}/phdProffesors/delete/${id}`);
  ...
  return (
    <div className="p-8">
      <h1 className="font-semibold text-2xl mb-6">PHD Proffesors Lead</h1>
