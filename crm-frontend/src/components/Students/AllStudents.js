...
const AllStudents = () => {
  ...
  const getStudents = async () => {
    ...
    try {
      ...
      if (data?.success) {
        const sortedStudents = data.students.sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-11 bg-[#2f2a7a] text-white mt-8 text-lg">
              ...
              <div className="col-span-2 p-4 font-bold text-left">Phone Number</div>
              <div className="col-span-2 p-4 text-xl font-bold text-left">College</div>
            ...
            <div className="max-h-[75vh] overflow-y-scroll">
              {students.map((student, index) => (
                <div className="grid grid-cols-11 bg-blue-200 border-2 border-b-gray-300 text-sm" key={index}>
                  <div className="col-span-2 p-4 text-left font-bold flex flex-col gap-4">
                    <h1>{student.name}</h1>
                  </div>
                  <div className="col-span-2 p-4 text-left font-bold">{student.email}</div>
                  <div className="col-span-2 p-4 pl-10 font-bold text-left">{student.phone}</div>
                  <div className="col-span-2 p-4 pl-10 font-bold text-left">{student.college}</div>
                  <div className="col-span-1 p-4 pl-10 font-bold text-left">{student.year}</div>
                  <div className="col-span-1 p-4 text-left font-bold">{student.date ? convertUTCtoIST(student.date) : "N/A"}</div>
